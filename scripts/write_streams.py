...
/* ── Component ──────────────────────────────────────────────────── */
export default function AnalyticsStreams() {
...
import os
path = os.path.join(os.path.dirname(__file__), '..', 'src', 'components', 'analytics', 'AnalyticsStreams.jsx')
path = os.path.normpath(path)
with open(path, 'w') as f:
    f.write(content.lstrip())
print(f"AnalyticsStreams.jsx written: {len(content.splitlines())} lines")
